...
interface FindNearbyZipsByCoordsRequest {
  lat: number;
  lon: number;
  radius: number;
}

interface FindNearbyZipsRequest {
  zip: string;
  radius: number;
}

interface PrinterWithDistance {
  id: string;
  distance: number;
  [key: string]: any;
}
...
const zipCodes: ZipCode[] = JSON.parse(jsonData);
...
export const findNearbyZipsByCoords = functions.https.onCall(
  async (data: FindNearbyZipsByCoordsRequest): Promise<PrinterWithDistance[]> => {
    ...
    const queryPoint: Coordinates = { lat, lon };
    
    // Use geokdbush to find nearby points
    const nearbyIndices = around(index, lon, lat, Infinity, radius * 1.60934) as number[];
    ...
    const printersWithDistance: PrinterWithDistance[] = [];
...
export const findNearbyZips = functions.https.onCall(
  async (data: FindNearbyZipsRequest): Promise<PrinterWithDistance[]> => {
    ...
    const queryPoint: Coordinates = { lat: sourceZip.lat, lon: sourceZip.lon };
    
    // Use geokdbush to find nearby points
    const nearbyIndices = around(index, sourceZip.lon, sourceZip.lat, Infinity, radius * 1.60934) as number[];
